...
def readMatFileTests(G_name):
    generators_f='ForTesting/'+G_name+'/gen_ims.mat'
    
    basis_f= ['ForTesting/'+G_name+'/'+name for name in os.listdir('ForTesting/'+G_name+'/') if name.endswith('.mat') and name!='gen_ims.mat']

    bases =[sio.loadmat(file)['basis'] for file in basis_f]
    
    orthobases=[0]*len(bases)
    
    for i in range(len(bases)):
        #
        # Orthogonalize basis: QR decomposition, Q part is a (partial) unitary
        Q, R = np.linalg.qr(bases[i])
        orthobases[i] = Q.T
        # print("orthobases[i][0]=",len(orthobases[i][0]),", orthobases[i]=",len(orthobases[i]))
    
    
    generators_long = sio.loadmat(generators_f)['gen_ims'] #-> loadmat gives dictionary, 
    
    gen_ims = [gen_im for gen_im in generators_long[0]]
    gen_names = [str(i) for i in range(len(gen_ims))]
    
    full = {'bases':orthobases, 'gen_names':gen_names, 'gen_images':gen_ims}
