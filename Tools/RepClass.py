...
class rep_by_generators(generators):
    ...
    def __init__(self, dimension, generatorSet = [], genImages = [], **kwargs):
        ...
        assert all([len(genImages[i])==self.dimension for i in len(genImages)]), "Images of wrong dimension."
        if len(genImages)>0:
            for i in range(len(genImages)):
                oneImage=group_element_image(generatorSet[i],genImages[i])
                self.Images.update(oneImage)
